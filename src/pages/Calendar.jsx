...
export default function Calendar() {
    ...
    if (day) {
      ...
      cells.push(
        <div key={ds} className={`cal-cell has-trades ${isGreen ? 'green' : 'red'}`}
          onClick={() => { setDayTrades(trades.filter(t => t.date === ds)); setDayTitle(`Trades — ${ds}`); setDayOpen(true) }}>
  ...
  return (
    <div className="main">
      ...
      <Modal open={dayOpen} onClose={() => setDayOpen(false)} title={dayTitle} maxWidth={900}>
        {dayTrades.length ? (
          <>
            <div className="table-wrap">
              <table>
                <thead><tr><th>#</th><th>Inst</th><th>Dir</th><th>Strike</th><th>Entry</th><th>T1 P&L</th><th>T2 P&L</th><th>Final P&L</th><th>Total P&L</th><th>Return %</th><th>B&R</th><th>EMA</th><th>Outcome</th><th>Notes</th></tr></thead>
                <tbody>
                  {dayTrades.map((t, i) => {
                    ...
                    return (
                      <tr key={t.id}>
                        ...
                        <td className="pnl-neu">{t.strike || '—'}</td>
                        ...
                        <td>{t.bnr || '—'}</td><td>{t.ema || '—'}</td>
                        <td><span className={`chip ${total > 0 ? 'chip-win' : total < 0 ? 'chip-loss' : 'chip-be'}`}>{total > 0 ? 'Win' : total < 0 ? 'Loss' : 'BE'}</span></td>
                        <td style={{ color: 'var(--text2)', maxWidth: 200, whiteSpace: 'normal', fontSize: 12 }}>{t.notes || '—'}</td>
                      </tr>
                    )
                  })}
                </tbody>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', marginTop: 16, fontSize: 14, color: 'var(--text2)' }}>
              Day Total: <span className={pc(dayTrades.reduce((s, t) => s + calcTrade(t).total, 0))} style={{ marginLeft: 10, fontSize: 16 }}>{fmt(dayTrades.reduce((s, t) => s + calcTrade(t).total, 0))}</span>
            </div>
          </>
